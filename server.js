...
function checkAuth(req, res, next) {
  ...
  if (headerKey === MY_API_KEY || bearerKey === MY_API_KEY) return next();
  return res.status(401).json({
    error: { message: 'Unauthorized. Provide x-api-key header or Authorization: Bearer <key>.' },
  });
}
...
// ---- Raw relay endpoint ----
// Built specifically to slot into an existing "multi-model fallback loop" style n8n
// workflow (e.g. one that already tries Gemini + several free OpenRouter models).
// It takes a fully-formed prompt (the caller already embedded its own instructions/
// JSON-schema request in the prompt text) and forwards it to Groq's free API, then
// returns Groq's response completely unmodified — which is already OpenAI-compatible
// ({ choices: [{ message: { content } }] } on success, { error: { message } } on
// failure) — so no special parsing logic is needed on the caller's side.
app.post('/api/generate-raw', checkAuth, async (req, res) => {
  try {
    ...
    if (!GROQ_API_KEY) {
      return res.status(500).json({
        error: { message: 'Server misconfigured: GROQ_API_KEY environment variable is not set.' },
      });
    }
    ...
    const data = await groqRes.json();
    // Pass Groq's response straight through, status code included.
